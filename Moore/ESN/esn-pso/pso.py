    ...
    def reset(self,
        ...
        for i in range(dimensions):
            if lower_bound[i] < upper_bound[i]:
                position.extend(np.random.uniform(lower_bound[i], upper_bound[i] + 1, 1))
            elif lower_bound[i] == upper_bound[i]:
                position.extend(np.array([lower_bound[i]))
            else:
                assert False
        ...
        self.velocity = [np.multiply(np.random.rand(dimensions), (upper_bound - lower_bound))]
