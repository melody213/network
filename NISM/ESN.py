...
class Reservoir(object):
    ...
    def __init__(self, n_internal_units=300, spectral_radius=1, leak=None,
...
res = Reservoir(n_internal_units=150, spectral_radius=1, leak=1,
                 connectivity=1, input_scaling=1, noise_level=0.0, circle=True)

input_repr = res.getReservoirEmbedding(X, pca, ridge_embedding,  n_drop=0, bidir=True, test = False)
print(input_repr)
input_repr_te = res.getReservoirEmbedding(sc_testdf, pca, ridge_embedding,  n_drop=0, bidir=True, test = True)
print(input_repr_te)
#input_repr = np.reshape(input_repr, (input_repr.shape[0], input_repr.shape[1], 1))
