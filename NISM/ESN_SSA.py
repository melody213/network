...
def initial_position(swarm_size=5, min_values = [20,0,0,0], max_values = [50,1,1,1]):
    position = np.zeros((swarm_size, 5))
    for i in range(0, swarm_size):
        for j in range(0, len(min_values)):
            position[i, j] = random.uniform(min_values[j], max_values[j])
        position[i, -1] = func(position[i, 0:position.shape[1] - 1])
    return position

def food_position(dimension = 4):
    food = np.zeros((1, dimension+1))
    for j in range(0, dimension):
        food[0,j] = 0.0
    food[0,-1] = func(food[0,0:food.shape[1]-1])
    return food
...
def update_position(position, food, c1 = 1, min_values = [20,0,0,0], max_values = [50,1,1,1]):
    for i in range(0, position.shape[0]):
...
def salp_swarm_algorithm(swarm_size = 5, min_values = [20,0.5,0.5,0.5], max_values = [50,1,1,1], iterations = 20):
    count = 0
...
ssa = salp_swarm_algorithm(swarm_size = 5, min_values = [20,0,0,0], max_values = [50,1,1,1], iterations = 20)