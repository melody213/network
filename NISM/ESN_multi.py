...
ridge_embedding = Ridge(alpha=10, fit_intercept=True, max_iter=3)
readout = Ridge(alpha=5, max_iter=3)

res1 = Reservoir(n_internal_units=80, spectral_radius=1, leak=1,
                 connectivity=1, input_scaling=1, noise_level=0.0, circle=False)

res2 = Reservoir(n_internal_units=80, spectral_radius=1, leak=1,
                 connectivity=1, input_scaling=1, noise_level=0.0, circle=False)

#res3 = Reservoir(n_internal_units=50, spectral_radius=1, leak=1,
#                 connectivity=1, input_scaling=1, noise_level=0.01, circle=False)

#res3 = Reservoir(n_internal_units=150, spectral_radius=0.6, leak=0.6,
#                 connectivity=0.25, input_scaling=0.1, noise_level=0.01, circle=False)


input_repr1 = res1.get_states(X, n_drop=5, bidir=True)
input_repr2 = res2.get_states(input_repr1, n_drop=5, bidir=True)

input_repr2 = res2.getReservoirEmbedding(input_repr2, pca, ridge_embedding,  n_drop=5, bidir=True, test = True)

input_repr_te1 = res1.get_states(sc_testdf, n_drop=5, bidir=True)
input_repr_te2 = res2.get_states(input_repr_te1, n_drop=5, bidir=True)

input_repr_te2 = res2.getReservoirEmbedding(input_repr_te2, pca, ridge_embedding,  n_drop=5, bidir=True, test = True)
readout.fit(input_repr2, traincat)
logits = readout.predict(input_repr_te2)
